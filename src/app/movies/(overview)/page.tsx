...
import { getMoviesByQuery } from "@/lib/db/movies";
import { Metadata } from "next";
import { parseQueryParamsToUrl, getSlugsForPagination } from "@/lib/util";
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const { nextPageSlug, previousPageSlug } = getSlugsForPagination("movies", params)
  ...
  return (
    <main className="px-4 md:px-6 py-8">
      ...
      <section className="grid grid-cols-1 grid-rows-[auto_1fr] space-y-4 md:space-y-0 md:grid-rows-none md:grid-cols-[auto_1fr]">
        <Filters genres={genres} type="movie" />
        <Suspense fallback={<GallerySkeleton />}>
          <ContentGallery type="movie" callback={() => getMoviesByQuery(params)} pagination={ { current: params.page || 1, next: nextPageSlug, previous: previousPageSlug } }  />
        </Suspense>
      </section>
    </main>
  )
